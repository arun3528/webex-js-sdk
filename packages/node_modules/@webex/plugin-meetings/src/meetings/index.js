...
// import {createClient, media, ReceiveSlot} from '@webex-connect/video-web-sdk';
...
import {
  ...
  AudioMain,
  createClient, media,
} from '@webex-connect/video-web-sdk';
...
export default class Meetings extends WebexPlugin {
  ...
  async connectCpaasSession(sessionId, sessionToken, clientId) {
    const localCamTrack = await media.createCameraTrack();
    const localMicTrack = await media.createMicrophoneTrack();

    this.cpaas.publishTrack(localCamTrack);
    this.cpaas.publishTrack(localMicTrack);


    // const remoteStream = new MediaStream();

    // Create the slots to receive audio and video.
    const receiveAudioSlot = await this.cpaas.createAudioReceiveSlot();
    const receiveVideoSlot = await this.cpaas.createVideoReceiveSlot();

    const audioRequest = new MediaRequest(
      Policy.ActiveSpeaker,
      new ActiveSpeakerInfo(100, false, false, true),
      [receiveAudioSlot]
    );
    const videoRequest = new MediaRequest(
      Policy.ActiveSpeaker,
      new ActiveSpeakerInfo(100, false, false, true),
      [receiveVideoSlot]
    );

    this.cpaas.requestMedia(AudioMain, [audioRequest]);
    this.cpaas.requestMedia(VideoMain, [videoRequest]);

    // Add the tracks to the stream when the remote media starts.
    receiveAudioSlot.on(ReceiveSlot.Events.MediaStarted, () => {
      Trigger.trigger(
        this,
        {
          file: 'meetings',
        ...
        'CPAAS_REMOTE_AUDIO',
        {
          remoteStream: receiveAudioSlot.stream
        }
      );
    });
    receiveVideoSlot.on(ReceiveSlot.Events.MediaStarted, () => {
      Trigger.trigger(
        this,
        {
          file: 'meetings',
        ...
        'CPAAS_REMOTE_VIDEO',
        {
          remoteStream: receiveVideoSlot.stream
        }
      );
    });

    await this.cpaas.joinSession(sessionId, sessionToken, clientId);
  }
